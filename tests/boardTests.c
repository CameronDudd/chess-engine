...
TEST_SETUP(boardTests) {
  initDistanceToEdgeLookup();
  fenPopulateBoard(FEN_STARTING_POSITION, testBoard);
}
...
TEST(boardTests, testWhiteMove2Str) {
  ...
  Move pawnMove = MOVE_ENCODE(52, 36, 0, 0);
  TEST_ASSERT_EQUAL_STRING("e4", move2str(pawnMove, board));

  fenPopulateBoard("rnbqkb1r/ppp1pppp/5n2/3pP3/8/8/PPPP1PPP/RNBQKBNR w KQkq - 0 1", board);
  Move pawnCaptureKnight = MOVE_ENCODE(28, 21, CAPTURE, 0);
  TEST_ASSERT_EQUAL_STRING("exf6", move2str(pawnCaptureKnight, board));

  fenPopulateBoard("r1bqkbnr/1ppp1ppp/p1n5/4p3/2B1P3/5N2/PPPP1PPP/RNBQ1RK1 b kq - 0 1", board);
  Move shortCastle = MOVE_ENCODE(60, 62, CASTLE, 0);
  TEST_ASSERT_EQUAL_STRING("O-O", move2str(shortCastle, board));

  fenPopulateBoard("rnbqk1nr/ppp1b1pp/3p4/4p1p1/4P1Q1/2NP4/PPP2PPP/2KR1BNR b kq - 0 1", board);
  Move longCastle = MOVE_ENCODE(60, 58, CASTLE, 0);
...
TEST(boardTests, testBlackMove2Str) {
  ...
  fenPopulateBoard("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1", board);
  Move pawnMove = MOVE_ENCODE(12, 28, 0, 0);
...
TEST(boardTests, testInitBitBoards) {
  ...
  TEST_ASSERT_EQUAL_UINT64(0x0000000000000081, piecePositionBitBoards[BLACK_ROOK_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x0000000000000042, piecePositionBitBoards[BLACK_KNIGHT_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x0000000000000024, piecePositionBitBoards[BLACK_BISHOP_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x0000000000000008, piecePositionBitBoards[BLACK_QUEEN_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x0000000000000010, piecePositionBitBoards[BLACK_KING_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x000000000000FF00, piecePositionBitBoards[BLACK_PAWN_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x8100000000000000, piecePositionBitBoards[WHITE_ROOK_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x4200000000000000, piecePositionBitBoards[WHITE_KNIGHT_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x2400000000000000, piecePositionBitBoards[WHITE_BISHOP_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x0800000000000000, piecePositionBitBoards[WHITE_QUEEN_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x1000000000000000, piecePositionBitBoards[WHITE_KING_KEY]);
  TEST_ASSERT_EQUAL_UINT64(0x00FF000000000000, piecePositionBitBoards[WHITE_PAWN_KEY]);
}
